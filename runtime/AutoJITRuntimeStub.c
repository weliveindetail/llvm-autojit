...
static int sps_read_uint64(const uint8_t **ptr, const uint8_t *end, uint64_t *value) {
    if (*ptr + 8 > end) {
...
static int sps_read_string(const uint8_t **ptr, const uint8_t *end, char **str, uint64_t *len) {
...
static int send_epc_message(int fd, const epc_message_t *msg) {
    ...
    if (write_all(fd, header, FD_MSG_HEADER_SIZE) < 0) {
        ret = -1;
    } else if (msg->arg_size > 0 && write_all(fd, msg->arg_bytes, msg->arg_size) < 0) {
        ret = -1;
    }
    ...
    if (result_msg.arg_size == 0) {
        ...
        *result_size = 0;
        ...
        if (result_msg.arg_size < 8) {
            ERROR_LOG("Result message too small: expected at least 8 bytes, got %zu\n", result_msg.arg_size);
            free_epc_message(&result_msg);
            return -1;
        }
...
static void cleanup_daemon(void) {
    if (g_daemon_fd >= 0) {
        /* Send hangup message */
        epc_message_t hangup = {
            .opcode = OPCODE_HANGUP,
            .seqno = 0,
            .tag_addr = 0,
            .arg_bytes = NULL,
            .arg_size = 0
...
void __llvm_autojit_register(const char *FilePath) {
    ...
    if (call_wrapper_function(g_daemon_fd, g_register_fn_addr,
                              args.data, args.size, &result, &result_size) < 0) {
...
void __llvm_autojit_materialize(void **GuidInPtrOut) {
    ...
    if (call_wrapper_function(g_daemon_fd, g_materialize_fn_addr,
                              args.data, args.size, &result, &result_size) < 0) {
