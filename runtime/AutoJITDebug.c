 ...
 * This is a minimal reimplementation of llvm/lib/ExecutionEngine/Orc/TargetProcess/JITLoaderGDB.cpp
 */
...
__attribute__((used))
cwrapper_function_result_t llvm_orc_registerJITLoaderGDBWrapper(const char *data, uint64_t size) {
    uint64_t start_addr;
    uint64_t range_size;
    int auto_register;

    if (parse_register_args(data, size, &start_addr, &range_size, &auto_register) < 0) {
...
__attribute__((used))
cwrapper_function_result_t llvm_orc_registerJITLoaderGDBAllocAction(const char *data, size_t size) {
